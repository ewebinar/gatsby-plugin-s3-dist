...
`);
        ...
        const isKeyInUse = {};
        const htmlFilesQueue = [];
        stream.on('data', ({ path, stats }) => {
            if (!stats.isFile()) {
                return;
            }
            const queueFn = async_1.asyncify(async () => {
                var _a, _b;
...
{dim   Created Redirect {cyan ${key}} => {cyan ${redirectLocation}}}\n`;
        ...
        await promisifiedParallelLimit(htmlFilesQueue, config.parallelLimit);
        if (config.removeNonexistentObjects) {
            const persistObjects = ((_b = config.retainObjectsPatterns) !== null && _b !== void 0 ? _b : []).map(glob => glob_to_regexp_1.default(glob, { globstar: true, extended: true }));
        ...
        if (process.env.AFTER_DEPLOY_HOOK_URL) {
            await node_fetch_1.default(process.env.AFTER_DEPLOY_HOOK_URL, {
                method: 'POST',
